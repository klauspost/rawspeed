...
The map and decoder can be deallocated once the metadata has been decoded. The `RawImage` will automatically be deallocated when it goes out of scope and the decoder has been deallocated. After that all data pointers that have been retrieved will no longer be usable.
...
To check if an image has been rotated, check `RawImage->fujiWidth` after calling `RawDecoder->decodeMetaData(...)`. If the return value is > 0, then the image has been rotated, and you can use this value to calculate the unrotated image size. See [here](https://rawstudio.org/svn/rawstudio/trunk/plugins/fuji-rotate/fuji-rotate.c) for an example on how to rotate the image back after de-mosaic.
...
This option will determine whether dither is applied when values are scaled to 16 bits. Dither is applied as a random value between `±scalefactor/2`. This will make it so that images fewer bits/pixel don't have a strong tendency for posterization, since values close to each other will be spaced out a bit.

Another way of putting it, is that if your camera saves 12 bits per pixel, when RawSpeed upscales this to 16 bits, the 4 "new" bits will be random instead of always the same value.
